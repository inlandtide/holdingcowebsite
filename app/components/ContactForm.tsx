...
export function ContactForm({ variant = "dark" }: { variant?: "dark" | "light" }) {
  ...
  if (status === "success") {
    return (
      <div className="gold-border px-6 py-8 text-center">
  ...
  return (
    <form onSubmit={handleSubmit} className="grid gap-5">
      <div>
        ...
        <input id="name" name="name" type="text" required value={formData.name} onChange={handleChange} placeholder="Your full name" className={`${fieldClass} px-4 py-3 text-sm`} />
      </div>
      <div className="grid gap-5 md:grid-cols-2">
        <div>
          ...
          <input id="email" name="email" type="email" required value={formData.email} onChange={handleChange} placeholder="your@email.com" className={`${fieldClass} px-4 py-3 text-sm`} />
        </div>
        <div>
          <label htmlFor="phone" className="mb-2 block text-xs font-medium uppercase tracking-[0.2em]" style={{ color: labelColor }}>
            Phone
          </label>
          <input id="phone" name="phone" type="tel" value={formData.phone} onChange={handleChange} placeholder="Optional" className={`${fieldClass} px-4 py-3 text-sm`} />
        </div>
      </div>
      <div>
        ...
        <textarea id="message" name="message" required rows={6} value={formData.message} onChange={handleChange} placeholder="Tell us about your business, opportunity, or partnership idea." className={`${fieldClass} resize-none px-4 py-3 text-sm`} />
      </div>
      {status === "error" && (
      ...
      <button
        ...
        className="border px-6 py-4 text-xs font-semibold uppercase tracking-[0.22em] transition disabled:opacity-60"
